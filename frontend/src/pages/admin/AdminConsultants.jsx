...
import { Users, Eye, Search, CheckCircle, XCircle, Stethoscope, FileText, Wallet, Clock, TrendingUp, MapPin, Building, Activity, Shield, Download, Lock, ShieldCheck } from 'lucide-react';
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
          ...
          <tbody className="divide-y divide-slate-50">
            {filtered.length === 0 ? (
              ...
              <tr key={c._id}
                ...
                onClick={() => protectedAction(c, 'view consultant details', () => setSelected(c))}
              >
                ...
                <td className="px-5 py-4">
                  <p className="text-xs text-slate-700 font-medium">{c.profile?.specialty || '—'}</p>
                </td>
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                    {['info@carebridgesystem.com', 'admin@carebridge.com', 'admin@carebridge.local'].includes(loggedInUser?.email) && (
                      <button 
                        onClick={() => setSetPassUser(c)}
                        title={c.hasRecordPassword ? "Password Protection Active (Click to Manage)" : "Set Record Access Password"}
                    ...
                    <button onClick={() => protectedAction(c, 'view consultant details', () => setSelected(c))}
                      className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-blue-600 transition-colors">
                      <Eye size={16} />
                    </button>
                    <button onClick={() => protectedAction(c, 'download consultant file', () => downloadPdf(`/exports/admin/consultants/${c._id}`, `Consultant_${(c.name || 'file').replace(/\s+/g, '_')}.pdf`))}
                      title="Download complete file"
                      className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-blue-600 transition-colors">
                      <Download size={16} />
                    </button>
                    <button
                      onClick={() => protectedAction(c, c.status === 'active' ? 'suspend consultant' : 'activate consultant', () => toggleStatus(c._id, c.status))}
            ...
            <div className="flex flex-col gap-4 border-b border-slate-100 pb-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => protectedAction(selected, selected.status === 'active' ? 'suspend consultant' : 'activate consultant', () => toggleStatus(selected._id, selected.status))}
                  ...
                  <button
                    type="button"
                    onClick={() => protectedAction(selected, 'edit consultant details', () => openConsultantEdit())}
                  ...
                  <button
                    type="button"
                    onClick={() => protectedAction(selected, 'download consultant file', () => downloadPdf(`/exports/admin/consultants/${selected._id}`, `Consultant_${(selected.name || 'file').replace(/\s+/g, '_')}.pdf`))}
                  ...
                  <button
                    onClick={() => protectedAction(selected, 'delete consultant record', () => handleDelete(selected._id))}
      ...
      <SetRecordPasswordModal
        ...
        onSuccess={(hasPassword) => {
          setConsultants(prev => prev.map(item => item._id === setPassUser._id ? { ...item, hasRecordPassword: hasPassword } : item));
